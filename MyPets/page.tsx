        ...
        {pets.map((p) => (
          <div key={p.id} className="bg-white p-4 md:p-5 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition">
            <div className="flex items-center gap-2 md:gap-3">
              {p.photo ? (
                <img src={p.photo as string} alt={p.name} className="w-10 h-10 md:w-12 md:h-12 rounded-full object-cover" />
              ) : (
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-2 md:p-4" onClick={(e) => e.stopPropagation()}>
            <div className="bg-white w-full max-w-md md:max-w-lg rounded-xl p-4 md:p-5 shadow-lg relative">
              ...
              <div className="flex flex-col items-center mb-3">
                {formData.photo ? (
                  <img src={formData.photo as string} alt="Pet" className="w-16 h-16 md:w-20 md:h-20 rounded-full mb-2 object-cover" />
                ) : (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-4">
                <div>
                  ...
                  <input name="name" value={formData.name} onChange={handleChange} className="w-full border rounded-lg p-1.5 md:p-2 mt-1 text-xs md:text-sm" placeholder="Pet name" />
                </div>
                <div>
                  <label className="text-xs md:text-sm font-medium">Species</label>
                  <select name="species" value={formData.species} onChange={handleChange} className="w-full border rounded-lg p-1.5 md:p-2 mt-1 text-xs md:text-sm">
                ...
                <div>
                  ...
                  <input name="breed" value={formData.breed} onChange={handleChange} className="w-full border rounded-lg p-1.5 md:p-2 mt-1 text-xs md:text-sm" />
                </div>
                <div>
                  <label className="text-xs md:text-sm font-medium">Date of Birth</label>
                  <div className="relative">
                    <input type="date" name="dob" value={formData.dob} onChange={handleChange} className="w-full border rounded-lg p-1.5 md:p-2 mt-1 text-xs md:text-sm" />
                ...
                <div>
                  ...
                  <input name="weight" value={formData.weight} onChange={handleChange} className="w-full border rounded-lg p-1.5 md:p-2 mt-1 text-xs md:text-sm" />
                </div>
                <div>
                  <label className="text-xs md:text-sm font-medium">Neutered/Spayed</label>
                  <div className="flex gap-1 md:gap-2 mt-1">
                    {["YES", "NO"].map(n => (
                      <button
                        key={n}
                        type="button"
                        className={`px-2 py-1 md:px-3 md:py-1 border rounded-lg text-xs md:text-sm ${formData.neutered === n ? "bg-emerald-100" : ""}`}
                  ...
                  <div className="flex gap-1 md:gap-2 mt-1">
                    {["Male", "Female", "Unknown"].map(g => (
                      <button
                        key={g}
                        type="button"
                        className={`px-2 py-1 md:px-3 md:py-1 border rounded-lg text-xs md:text-sm ${formData.gender === g ? "bg-emerald-100" : ""}`}
                ...
                <div className="col-span-2">
                  ...
                  <textarea name="conditions" value={formData.conditions} onChange={handleChange} className="w-full border rounded-lg p-1.5 md:p-2 mt-1 text-xs md:text-sm" rows={2} placeholder="Describe conditions..." />
                </div>
              </div>

              <div className="flex justify-end gap-2 md:gap-3 mt-4 md:mt-6">
                <button onClick={() => setOpenModal(false)} className="px-3 py-1.5 md:px-4 md:py-2 border rounded-lg text-xs md:text-sm">Cancel</button>
                <button onClick={handleSubmit} className="px-3 py-1.5 md:px-4 md:py-2 bg-emerald-500 text-white rounded-lg text-xs md:text-sm">
